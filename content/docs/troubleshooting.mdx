...
- **Too many worker goroutines** — `WorkerPool` defaults to `10`. Tune it for your workload; sends are mostly I/O-bound, so a larger pool is usually fine, but a huge pool plus a large queue raises memory.
