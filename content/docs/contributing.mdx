...
- **Message builders live with their family** — e.g., `channel/chat/message.go`, `channel/mail/message.go`.
- **Raw HTTP only.** Do not introduce third-party SDKs. See [Security & Redaction](/docs/features/security) for why.
- **Redact secrets in logs** with `middleware.Redact` / `middleware.RedactAttr`. Never log raw API keys or tokens.
...
2. Config struct that keeps secrets out of logs (redact via `middleware.Redact` when logging).
