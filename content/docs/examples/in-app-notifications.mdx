...
dbChannel := notifier.Channel("database").(*database.Channel)
```

This creates a `notifications` table with columns: `id`, `type`, `notifiable_type`, `notifiable_id`, `title`, `body`, `data` (JSON), `read_at`, `created_at`.
