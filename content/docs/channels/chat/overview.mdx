...
Multiple Slack workspaces, multiple Teams tenants, multiple Discord servers — register each under its own channel name via `Config.Name`:

```go
notifier.RegisterChannel(slack.New(slack.Config{Name: "slack-eng",   WebhookURL: engURL}))
notifier.RegisterChannel(slack.New(slack.Config{Name: "slack-ops",   WebhookURL: opsURL}))
notifier.RegisterChannel(slack.New(slack.Config{Name: "slack-sales", WebhookURL: salesURL}))
```
