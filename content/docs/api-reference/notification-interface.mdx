...
For channels beyond the built-in ones, implement the optional `ChannelFormatter` interface. The notifier calls it only when no per-channel interface matches:
